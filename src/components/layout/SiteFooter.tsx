...
export function SiteFooter() {
  return (
    <footer className="border-t-4 border-violet-600 bg-indigo-900 text-gray-200">
      <div className="mx-auto max-w-7xl px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-12">
        {/* About Campus */}
        <div>
          <div className="flex items-center gap-3 mb-4">
            <img src="/logo.png" alt={`${site.name} Logo`} className="w-10 h-10 rounded-lg object-contain bg-white/10" />
            <div className="text-xl font-bold text-white">{site.fullName}</div>
          </div>
          <p className="mt-2 text-base font-bold text-gray-200">
            {site.location}. Affiliated to{' '}
            <a
              className="font-bold text-violet-400 underline underline-offset-4 hover:no-underline"
        ...
        {/* Quick Links */}
        <div>
          <div className="text-base font-bold text-violet-400 uppercase tracking-wider mb-4">Quick Links</div>
          <ul className="grid gap-3 text-base font-bold">
            {site.nav.slice(0, 6).map((i) => (
              <li key={i.href}>
                <Link className="hover:text-violet-400 transition-colors" href={i.href}>
                  {i.label}
                </Link>
              </li>
            ))}
        ...
        {/* Admissions */}
        <div>
          <div className="text-base font-bold text-violet-400 uppercase tracking-wider mb-4">Admissions</div>
          <ul className="grid gap-3 text-base font-bold">
            <li>
              <Link className="hover:text-violet-400 transition-colors" href="/admissions">
                Apply Now
              </Link>
            </li>
            <li>
              <Link className="hover:text-violet-400 transition-colors" href="/courses">
                Explore Courses
              </Link>
            </li>
            <li>
              <Link className="hover:text-violet-400 transition-colors" href="/contact">
                Contact Admissions
              </Link>
            </li>
          </ul>
        ...
        <div>
          <div className="text-base font-bold text-violet-400 uppercase tracking-wider mb-4">Contact Information</div>
          <ul className="grid gap-3 text-base font-bold">
            ...
            <li className="flex items-start gap-2">
              <span>Email:</span>
              <a className="hover:underline underline-offset-4 text-violet-400" href="mailto:info@jayaprithivicampus.edu.np">
                info@jayaprithivicampus.edu.np
              </a>
            </li>
          </ul>
          <div className="mt-6 flex gap-4">
            <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="hover:text-violet-400 transition-colors">
            ...
            <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="hover:text-violet-400 transition-colors">
            ...
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="hover:text-violet-400 transition-colors">
      ...
      <div className="border-t border-white/10 py-6 text-center text-xs text-gray-400 font-bold">
